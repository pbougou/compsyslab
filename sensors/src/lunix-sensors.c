...
#include "lunix.h"
extern long lookup_light[], lookup_voltage[], lookup_temperature[];
/*
 * Flexible arrays: https://gcc.gnu.org/onlinedocs/gcc/Zero-Length.html
 */
	...
	uint16_t batt, uint16_t temp, uint16_t light)
{
	...
	printk(KERN_DEBUG "lunix_sensor_update: battery = %ld\n", lookup_voltage[batt]);
	s->msr_data[TEMP]->values[0] = temp;
	printk(KERN_DEBUG "lunix_sensor_update: temperature = %ld\n", lookup_temperature[temp]);
	s->msr_data[LIGHT]->values[0] = light;
	printk(KERN_DEBUG "lunix_sensor_update: light = %ld\n", lookup_light[light]);
